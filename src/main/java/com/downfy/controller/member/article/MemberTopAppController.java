...
@RequestMapping("/member/topapp")
@Controller
public class MemberTopAppController extends AbstractController {

    private final Logger logger = LoggerFactory.getLogger(MemberTopAppController.class);
    @Autowired
    ...
    public String index(Device device, Model uiModel) {
        try {
            return view(device, "member/topapp/index");
        } catch (Exception ex) {
            logger.error("Cannot view list top apps.", ex);
        }
